...
document.addEventListener('DOMContentLoaded', () => {
  initPage('userManagement');
  const _user = getUser();
  if (!_user || _user.role !== 'HRD') {
...
function renderUsers(data) {
  ...
  container.innerHTML = data.map((u, i) => {
    ...
    const statusLabel = {
      ACTIVE:'Aktif', PENDING:'Menunggu', REJECTED:'Ditolak', INACTIVE:'Nonaktif'
    }[u.status] || u.status;
    ...
    : '<span style="font-size:11px;color:var(--text-secondary)">Akun Anda</span>';
