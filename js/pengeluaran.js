...
let camStream = null, camTarget = null, facingMode = 'environment';
...
// ============================================================
// FORMAT NOMINAL — Titik ribuan otomatis
// ============================================================
...
// NAMA DROPDOWN
// ============================================================
let namaMode = 'dropdown';

function onNamaInput(val) {
  if (namaMode === 'custom') return;
...
function onUraianInput(val) {
  // Auto uppercase
  const el = document.getElementById('fUraian');
  if (el) {
  ...
  if (matched.length) {
    matched.forEach(m => {
      html += `<div class="pos-dd-item highlight" onclick="selectPos('${m.idPos}','${m.namaPos.replace(/'/g, "\\'")}')">
        ...
        <div><div class="pos-dd-name">${m.namaPos}</div></div>
    ...
    allPos.slice(0, 6).forEach(p => {
      html += `<div class="pos-dd-item" onclick="selectPos('${p.id}','${p.namaPos.replace(/'/g, "\\'")}')">
        <div><div class="pos-dd-name">${p.namaPos}</div><div class="pos-dd-cat">${p.kategori || ''}</div></div>
      </div>`;
    });
  }
...
function selectPos(idPos, namaPos) {
  ...
  if (fPosId) fPosId.value = idPos;

  const posChipName = document.getElementById('posChipName');
  if (posChipName) posChipName.textContent = namaPos;

  const posSelectedBox = document.getElementById('posSelectedBox');
  if (posSelectedBox) posSelectedBox.style.display = 'block';

  const posManualBox = document.getElementById('posManualBox');
  if (posManualBox) posManualBox.style.display = 'none';

  const oldChip = document.getElementById('posSelectedChip');
  if (oldChip) {
    const posSelectedName = document.getElementById('posSelectedName');
    if (posSelectedName) posSelectedName.textContent = namaPos;
    oldChip.classList.add('show');
  }
...
function handlePosExtras(name) {
  ...
  const isBBM   = namaPos.includes('bbm') || namaPos.includes('bahan bakar')
                || namaPos.includes('solar') || namaPos.includes('pertalite');
  const isPrw   = namaPos.includes('perawatan') || namaPos.includes('servis');
  const isPajak = namaPos.includes('pajak kendaraan') || namaPos.includes('stnk')
                || namaPos.includes('kir') || namaPos === 'pajak';
...
function removeFile(type) {
  if (type === 'nota') {
    ...
    const el = document.getElementById('notaFileInput');
    if (el) el.value = '';
  } else {
    ...
    const el = document.getElementById('buktiFileInput');
    if (el) el.value = '';
  }
}

function retakePhoto(type) { removeFile(type); }
...
async function openCamera(target) {
  ...
  if (camTitle) camTitle.textContent = target === 'nota' ? '📸 Ambil Foto Nota' : '📸 Ambil Foto Bukti Transfer';
...
async function startCamera() {
  ...
  try {
    ...
    if (!errEl) return;
    if (err.name === 'NotAllowedError')     errEl.textContent = '⛔ Izin kamera ditolak.';
    else if (err.name === 'NotFoundError')  errEl.textContent = '📷 Kamera tidak ditemukan.';
    else                                    errEl.textContent = '❌ Gagal membuka kamera: ' + err.message;
...
function capturePhoto() {
  ...
  if (!camStream || !video.videoWidth) { showToast('Kamera belum siap.', 'warning'); return; }
  canvas.width  = video.videoWidth;
  canvas.height = video.videoHeight;
  canvas.getContext('2d').drawImage(video, 0, 0);
  const src  = canvas.toDataURL('image/jpeg', 0.92);
  const b64  = src.split(',')[1];
  setMediaPreview(camTarget, src, b64, 'image/jpeg');
  closeCamera();
...
// ============================================================
// SAVE — Full dengan sync BBM / Perawatan / Pajak
// ============================================================
async function saveDataFull() {
  ...
  try {
    ...
    const r = await callAPI(id ? 'updatePengeluaran' : 'addPengeluaran', {
      id, timestamp: ts, nama,
      uraian: uraian.toUpperCase(), // pastikan uppercase sampai ke backend
      idPos, namaPos,
      nominal,                      // FIX: sudah angka murni dari getNominalRaw
      metodePembayaran: document.getElementById('fMetode')?.value || 'Tunai',
      noPolisi        : nopol,
      idArmada        : armSel?.value || '',
    ...
    if (bbmShown && !id) {
      ...
      const lit      = parseFloat(document.getElementById('fLiter')?.value)  || 0;
      const armBBM   = document.getElementById('fArmada');
      const noPolBBM = armBBM?.options[armBBM?.selectedIndex]?.dataset.nopol || '';
      const rBBM = await callAPI('addBBM', {
        tanggal: ts.split('T')[0], idArmada: armBBM?.value || '',
        noPolisi: noPolBBM, jenisBBM: document.getElementById('fJenisBBM')?.value || '',
        liter: lit, hargaPerLiter: harga
      });
    ...
    if (prwShown && !id) {
      ...
      const rPrw = await callAPI('addPerawatan', {
        tanggal: ts.split('T')[0], idArmada: armPrw?.value || '',
        noPolisi: noPolPrw, jenisPerawatan: document.getElementById('fJenisPrw')?.value || '',
        biaya: nominal, keterangan: document.getElementById('fPrwKet')?.value || '',
...
function editRow(row) {
  ...
  if (fTimestamp) fTimestamp.value = row.timestamp ? row.timestamp.replace(' ', 'T').substring(0, 16) : '';

  namaMode = 'custom';
  const fNama = document.getElementById('fNama');
  if (fNama) fNama.value = row.nama;
  const namaIsCustom = document.getElementById('namaIsCustom');
  if (namaIsCustom) namaIsCustom.style.display = 'block';

  const fUraian = document.getElementById('fUraian');
  if (fUraian) fUraian.value = (row.uraian || '').toUpperCase();

  // FIX: tampilkan nominal dengan format titik ribuan saat edit
  const fNominal = document.getElementById('fNominal');
  if (fNominal) fNominal.value = row.nominal ? parseInt(row.nominal).toLocaleString('id-ID') : '';

  const fMetode = document.getElementById('fMetode');
  if (fMetode) fMetode.value = row.metodePembayaran || 'Tunai';
...
function onJenisBBMChange() {
  ...
  if (price > 0 && hargaEl) {
    hargaEl.value = price;
    if (refChip)  { refChip.textContent = 'Ref: Rp ' + price.toLocaleString('id-ID') + '/L'; refChip.style.display = 'inline'; }
    if (resetBtn)   resetBtn.style.display = 'none';
    if (noteEl)   { noteEl.textContent = '✅ Harga referensi Pertamina Juni 2026 — bisa direvisi manual'; noteEl.style.color = 'var(--success)'; }
  } else if (hargaEl) {
    hargaEl.value = '';
    hargaEl.placeholder = 'Masukkan harga per liter...';
    if (refChip)  refChip.style.display = 'none';
    if (resetBtn) resetBtn.style.display = 'none';
    if (noteEl)   { noteEl.textContent = 'Masukkan harga per liter secara manual'; noteEl.style.color = 'var(--text-secondary)'; }
  }
  updateBBMCalc();
}
...
function onHargaInput() {
  ...
  const resetBtn = document.getElementById('btnResetHarga');
  if (resetBtn) {
    resetBtn.style.display = (currentRefHarga > 0 && price !== currentRefHarga) ? 'flex' : 'none';
  }
  updateBBMCalc();
}
...
function resetHargaToRef() {
  if (currentRefHarga > 0) {
    const hargaEl = document.getElementById('fHargaL');
    if (hargaEl) hargaEl.value = currentRefHarga;
    const resetBtn = document.getElementById('btnResetHarga');
    if (resetBtn) resetBtn.style.display = 'none';
    updateBBMCalc();
  }
...
function updateBBMCalc() {
  ...
  if (summary) {
    if (nominal > 0 && harga > 0) {
      ...
      const sumNominal = document.getElementById('bbmSumNominal');
      const sumHarga   = document.getElementById('bbmSumHarga');
      const sumLiter   = document.getElementById('bbmSumLiter');
      if (sumNominal) sumNominal.textContent = formatRupiah(nominal);
      if (sumHarga)   sumHarga.textContent   = 'Rp ' + harga.toLocaleString('id-ID') + '/L';
      if (sumLiter)   sumLiter.textContent   = liter.toFixed(2) + ' L';
    } else {
...
async function onArmadaPajakChange() {
  ...
  const pajakCurrentStatus = document.getElementById('pajakCurrentStatus');
  if (!id) { if (pajakCurrentStatus) pajakCurrentStatus.style.display = 'none'; return; }
  ...
  const currSTNK  = document.getElementById('currSTNK');
  const currKIR   = document.getElementById('currKIR');
  const currPajak = document.getElementById('currPajak');
  if (currSTNK)  currSTNK.textContent  = d.tanggalSTNK  || 'Belum diisi';
  if (currKIR)   currKIR.textContent   = d.tanggalKIR   || 'Belum diisi';
  if (currPajak) currPajak.textContent = d.tanggalPajak || 'Belum diisi';
  ...
  const statSTNK  = document.getElementById('statSTNK');
  const statKIR   = document.getElementById('statKIR');
  const statPajak = document.getElementById('statPajak');
  if (statSTNK)  statSTNK.innerHTML  = renderStat(d.statusSTNK);
  if (statKIR)   statKIR.innerHTML   = renderStat(d.statusKIR);
  if (statPajak) statPajak.innerHTML = renderStat(d.statusPajak);

  if (sel) sel.dataset.pajakId = d.id || '';
  if (pajakCurrentStatus) pajakCurrentStatus.style.display = 'block';
}
