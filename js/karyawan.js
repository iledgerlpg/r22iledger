...
function initKaryawanPage() {
  // 1. Ambil session user secara aman lewat fungsi helper lu
  _user = getUser(); 

  // 2. Bersihkan string role untuk menghindari typo kapitalisasi/spasi
  const currentRole = (_user && _user.role) ? _user.role.toString().trim().toUpperCase() : '';

  // 3. Proteksi Halaman: Hanya akun ber-role HRD yang boleh lolos
  if (currentRole !== 'HRD') {
    ...
    return; // Stop di sini, jangan tarik data dari database
  }
  
  // 4. Kalau rolenya HRD, langsung tarik data karyawan
  loadData();
}
...
async function saveData() {
  ...
  let tanggalGajiBerlaku = document.getElementById('fTglMasuk').value; 
...
/**
 * UTILITY HELPER: Konversi DD/MM/YYYY -> YYYY-MM-DD
 */
